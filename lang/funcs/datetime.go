...
// TimeCmp compares two timestamps, returning true if first timestamp if after second, otherwise false.
//
// In the Terraform language, timestamps are conventionally represented as
...
//
// The result is a boolean, which tells if first timestamp is after second one
